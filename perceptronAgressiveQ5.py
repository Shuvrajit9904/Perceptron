...
def perceptron(dataset, weight, bias, mu, featuresize):    
    for line2 in dataset:        
        y = int(line2[0])
        x = np.asarray(line2[1:])
        factor = y * (np.inner(x,weight)+bias)
        denom = np.inner(x,x) + 1
        eta = float(mu - factor)/float(denom)
        if factor < mu:            
...
def updatecount(dataset, weight, bias, mu, featuresize):
    update = 0
    for line2 in dataset:        
        y = int(line2[0])
        x = np.asarray(line2[1:])
    ...
    #    print "Cross Validation for eta = ",eta
        ...
        for i3 in range(k):            
            ...
            for ep in range(epoch):
                ...
                weight = perceptron(train,initialweight, bias, mu, featuresize)
        ...
        print "For mu : {}, Average Accuracy = {}".format(mu, avgacc)            
        if avgacc >= maxaccuracy:
            ...
            #out.append(mu)
        #print '\n'
    print "(a) The best Hyperparameter: Mu = {} ".format(out)
...
epoch = 10
mu = crossvalidation(5, hyperparamEta, featuresize, epoch, hyperparamMu)
...
for ep in range(20):    
    ...
    weight = perceptron(t,initialweight, bias, mu, featuresize)
    u = updatecount(t,initialweight, bias, mu, featuresize)
    updt.append(u)
    initialweight = weight[1:]    
