...
def perceptron(dataset, weight, bias, eta, mu, featuresize):
    avgwt = [0.001]*featuresize
    np.asarray(avgwt)
    avgbias = 0    
    for line2 in dataset:        
        ...
        avgwt = avgwt + weight
        avgbias += bias    
    avgbias = np.asarray(avgbias)    
    return np.append(avgbias,avgwt)

def updatecount(dataset, weight, bias, eta, mu, featuresize):
    update = 0
    avgwt = [0.001]*featuresize
    np.asarray(avgwt)
    avgbias = 0    
        ...
        for eta in hyperparamEta:
            ...
            for i3 in range(k):            
                ...
                for ep in range(epoch):
                    ...
                    weight = perceptron(train,initialweight, bias, neweta, mu, featuresize)
...
for ep in range(20):    
    ...
    weight = perceptron(t,initialweight, bias, eta, mu, featuresize)
    u = updatecount(t,initialweight, bias, eta, mu, featuresize)
    updt.append(u)
    initialweight = weight[1:]    
...
print "Maximum Dev Set accuracy:{} for Epoch #{}".format(mxdevaccr, epo)
print "Dev Set accuracy for last Epoch {}".format(accuracy)
print "Total updates on training dataset from Epoch #1 to Best Epoch({}) is {}".format(epo,updatesum)
...
'''
#Plot Graphs: Accuracy Vs Epochs
