...
def perceptron(dataset, weight, bias, eta, featuresize):
    ...
    for line2 in dataset:        
        ...
        if factor < 0:
            ...
            bias = bias + (eta * y)
        np.asarray(weight)                
        avgwt = np.add(avgwt, weight)        
        avgbias += bias    
    avgbias = np.asarray(avgbias)    
    return np.append(avgbias,avgwt)

def updatecount(dataset, weight, bias, eta, featuresize):
    update = 0
    avgwt = [0.001]*featuresize
    np.asarray(avgwt)
    avgbias = 0    
    ...
    for eta in hyperparamEta:
        ...
        for i3 in range(k):            
            ...
            for ep in range(epoch):
                ...
                weight = perceptron(train,initialweight, bias, neweta, featuresize)
...
for ep in range(20):    
    ...
    initialweight = [0.001]* featuresize
    bias = 0.001
    weight = perceptron(t,initialweight, bias, eta, featuresize)
    #initialweight = weight[1:]
    #bias = weight[0]
    u = updatecount(t,initialweight, bias, eta, featuresize)
    updt.append(u)    
